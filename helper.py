...
def most_common_words(selected_user, df):
    ...
    for message in temp["message"]:  # use temp not df
        for word in message.lower().split():  # split is important
            if word not in stop_words:
                words.append(word)
...
def monthly_timeline(selected_user, df):
    ...
    df['month_num'] = df['month'].map(months_dict)
 ...
 # def sentiment(df):
 #
 #    df['postive'] = [sentiments.polarity_scores(i)["pos"] for i in df["message"]]
